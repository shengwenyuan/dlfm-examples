        ...
        for subject in subjects:
            ...
            for date in dates[:1]:
                ...
                for session in sessions:
                    ...
                    choice_file = os.path.join(alf_path, "_ibl_trials.choice.npy")
                    contrast_right_file = os.path.join(alf_path, "_ibl_trials.contrastRight.npy")
                    contrast_left_file = os.path.join(alf_path, "_ibl_trials.contrastLeft.npy")
                    if not (os.path.exists(choice_file) and 
                            os.path.exists(contrast_right_file) and 
                            os.path.exists(contrast_left_file)):
                        continue
                    
                    choice = np.load(choice_file)
                    choice = (choice == 1).astype(int) # -1,0 -> 0, 1 -> 1
                    contrast_right = np.load(contrast_right_file)
                    contrast_left = np.load(contrast_left_file)
                    contrast_right = np.nan_to_num(contrast_right, nan=0.0)
                    contrast_left = np.nan_to_num(contrast_left, nan=0.0)
                    
                    min_length = min(len(choice), len(contrast_right), len(contrast_left))
                    if min_length < 2:
                        continue
                        
                    choice = choice[:min_length]
                    contrast_right = contrast_right[:min_length]
                    contrast_left = contrast_left[:min_length]
                    
                    stimulus = contrast_right - contrast_left
                    bias = np.ones(min_length)
                    
                    prev_choice = np.zeros(min_length)
                    prev_choice[1:] = choice[:-1]
                    
                    prev_contrast_right = np.zeros(min_length)
                    prev_contrast_left = np.zeros(min_length)
                    prev_contrast_right[1:] = contrast_right[:-1]
                    prev_contrast_left[1:] = contrast_left[:-1]
                    prev_stimulus_side = np.zeros(min_length)
