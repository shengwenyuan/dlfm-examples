...
npy_dir = os.path.join(root, "output", "results_IOHMM", "mcmc")
graph_dir = os.path.join(root, "figs", "rmse_weights")
...
def plot_rmse_w():
    ...
    for seed in np.arange(num_repeats):
        error_in_weights_random = np.load(os.path.join(npy_dir, "random_gibbs_PG_errorinweights_atseed"+str(seed)+"_gibbs_400.npy"))
        ...
        error_in_weights_dlfm = np.load(os.path.join(npy_dir, "dlfm_errorinweights_atseed"+str(seed)+".npy"))
    ...
    plt.grid()
    # plt.show()
    plt.savefig(os.path.join(graph_dir, "rmse_weights.png"), dpi=400)
